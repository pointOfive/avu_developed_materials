...
A while ago, before I was married, I felt it was important for me to get married 
sooner rather than later because I was worried that the later in life I waited 
to have a child, the more risky the birth would be for adverse birth outcomes. I
remember being told -- by my mother, I think -- that because I was a guy I
didn't have to worry about these things: having a child when you were older was 
...
topic](HEATMAP/41380_2016_Article_BFmp201570_Fig3_HTML.jpg)
...
wanted to recreate, but the data source they used to perform their analysis is 
(perhaps both unimpressively and impressively?) turns out to just be publicly 
...
- `readr::read_table2` (which for the same reasons as above would not have worked)
...
and *then* I was able to use `readr::read_table2` (or could have even just 
`readr::read_table`) to extract the columns (separated by whitespace) that I 
had extracted. *Note that there are also* ways to read exact-location positioned 
data with the `readr::` library functions, but the `cut -> readr::read_table2` 
...
parent_ages_outcomes <- readr::read_table2(d1, col_names=FALSE)
...
advese_reported_fig <- parent_ages_outcomes %>% 
  ...
  layout(title = 'Number of congenital anomaly birth outcomes in the US in 2019',
         ...
         margin = list(l=50, r=50, t=75, b=50))
...
no_advese_reported_fig <- parent_ages_outcomes %>% 
  ...
  dplyr::filter(paternal_age<50) %>%
  dplyr::filter(maternal_age>13) %>%
  plot_ly(x=~paternal_age, y=~maternal_age) %>% 
  add_histogram2d()  %>%
  layout(title = 'Number of non congenital anomaly outcomes in the US in 2019',
         xaxis = list(title = 'Paternal Age'),
         yaxis = list(title = 'Maternal Age'),
         margin = list(l=50, r=50, t=75, b=50))
        
no_advese_reported_fig
...
- Now we can plot this as a *heat map*, here visulized as a *surface plot*.
...
plot_ly(x=~paternal_age, y=~maternal_age, z=~abnormal_percent) %>% 
  ...
  layout(scene=list(xaxis = list(title = 'Paternal Age'),
                    yaxis = list(title = 'Maternal Age'),
                    zaxis = list(title = 'Abnormal %'))) %>% 
  colorbar(title = "Abnorm%")
